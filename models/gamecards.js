...
const CREATE = 'INSERT INTO game_cards(gameid, cardid) SELECT $1, c.cardid FROM cards c RETURNING *'

const DELETE_GAME = 'DELETE FROM game_cards WHERE gameid=$1'
...
const FIND_NUMBER_OF_CARDS_BY_USERID = 'SELECT COUNT(*) AS count FROM game_cards WHERE gameid=$1 AND userid=$2 AND cardid IS NOT NULL'
...
module.exports = {
  ...
  deleteUser: function(userid, gameid) {
    return db.any( DELETE_USER, [userid, gameid] )
  },
  ...
  findNumberOfCardsByUserId: function(gameid, userid) {
    return db.any( FIND_NUMBER_OF_CARDS_BY_USERID, [gameid, userid] )
  },
